...
#ifdef NeurNET
    NN *NeurNet; /*pointer to object NN*/
#endif
};
...
inline void Vertex::fix_var_i(){/*fix variable node to rue or false*/
    if(NeurNet->Predict(prod_V_plus, prod_V_minus, _surveys_cl_to_i_plus.size(), _surveys_cl_to_i_minus.size())) _who_I_am=true;
